 ...
 *
 * Assembles the stylesheet from source, the same way stat-css.test.ts does and
 * for the same reason: reading dist/ passes locally and fails from a clean tree
 * (CI runs `test` before `build`), and this must not run under jsdom, where the
 * global URL resolves relative imports against the document's base instead of
 * import.meta.url.
 *
