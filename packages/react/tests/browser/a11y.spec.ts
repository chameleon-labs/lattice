...
for (const family of families) {
  ...
  for (const theme of THEMES) {
    test(`${title} has no axe violations in ${theme}`, async ({page, request}) => {
      const stories = (await fetchStories(request)).filter((story) => story.title === `Components/${title}`);
...
test('borders survive forced-colors', async ({page}) => {
  ...
  const width = await page.locator('.lat-button').evaluate((el) => getComputedStyle(el).borderTopWidth);
...
test('the switch off and on states are visually distinct under forced-colors', async ({page}) => {
  await page.emulateMedia({forcedColors: 'active'});
