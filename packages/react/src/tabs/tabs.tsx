...
export function TabList<T extends ElementType = 'div'>({
  ...
  return (
    <AriakitTabList
      {...props}
      className={className === undefined ? 'lat-tab-list' : `lat-tab-list ${className}`}
    />
  )
}

export function Tab<T extends ElementType = 'button'>({ className, ...props }: TabProps<T>) {
  return (
    <AriakitTab
      {...props}
      className={className === undefined ? 'lat-tab' : `lat-tab ${className}`}
    />
  )
}
...
export function TabPanel<T extends ElementType = 'div'>({
  ...
  return (
    <AriakitTabPanel
      {...props}
      className={className === undefined ? 'lat-tab-panel' : `lat-tab-panel ${className}`}
    />
  )
}
