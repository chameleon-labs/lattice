...
import {NARROW_HEADING_SIZES, TYPOGRAPHY_BREAKPOINT_REM, TYPOGRAPHY_ROLES} from '../../config/typography-roles.js';
...
test('the breakpoint follows the user default font size', async ({page}) => {
  await page.setContent(
    documentWith(RESPONSIVE_ROLES.map((role) => `<div data-role="${role}">${role}</div>`).join('')),
  );
  ...
  const root = Number.parseFloat(await page.evaluate(() => getComputedStyle(document.documentElement).fontSize));
  const viewportWidth = page.viewportSize()?.width ?? 0;
  const narrow = viewportWidth < TYPOGRAPHY_BREAKPOINT_REM * root;

  expect(
    await page.evaluate((breakpoint) => matchMedia(`(width < ${breakpoint}rem)`).matches, TYPOGRAPHY_BREAKPOINT_REM),
  ).toBe(narrow);
