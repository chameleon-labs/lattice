...
import {DTCG_SCHEMA, emitCss, emitTokens, formatOklch, type ColorToken} from '../generate/emit.js';
import {ELEVATION_ROLE_COUNT, SHADOW_PRIMITIVE_COUNT, elevationCss} from '../generate/elevation.js';
import {LAYOUT_PRIMITIVE_COUNT, LAYOUT_PRIMITIVE_COUNTS, layoutCss} from '../generate/layout.js';
...
describe('lattice.css', () => {
  ...
  it('emits no numbered scale steps', () => {
    expect(emitCss()).not.toMatch(/--lat-(gray|accent|danger|warning|success|info|decorative)-\d+:/);
  });
  ...
  it('declares theme-independent primitives once before every themed block', () => {
    ...
    expect(count(css)).toBe(GLOBAL_DECLARATIONS + PER_BLOCK * BLOCKS + TYPOGRAPHY_RESPONSIVE_OVERRIDE_COUNT);
...
describe('tokens.json', () => {
  ...
  const colorLeaves = (): {path: string; token: ColorToken}[] =>
    leaves().filter((leaf) => leaf.token.$type === 'color' && typeof leaf.token.$value === 'object') as unknown as {
      path: string;
      token: ColorToken;
    }[];
  ...
  it('keeps CSS and DTCG aliases in parity for every typography role', () => {
    ...
    for (const [roleName, token] of Object.entries(text)) {
      ...
      for (const [property, cssProperty] of Object.entries(properties)) {
        ...
        expect(css).toContain(`--lat-text-${roleName}-${cssProperty}: var(--lat-${primitive});`);
      }
    }
  });
});
