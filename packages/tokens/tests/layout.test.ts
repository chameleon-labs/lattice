...
import {LAYOUT_PRIMITIVE_COUNT, LAYOUT_PRIMITIVE_COUNTS, layoutCss, layoutTokens} from '../generate/layout.js';
  ...
  it('keeps CSS and DTCG names and values in parity', () => {
    ...
    >) {
      for (const [tokenName, token] of Object.entries(group)) {
        ...
        expect(css).toContain(`--lat-${groupName}-${tokenName}: ${token.$value.value}${token.$value.unit};`);
      }
    }
  });
});
