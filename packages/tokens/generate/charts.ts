...
export function validateCategorical(
  ...
  const tritan = list.length
    ? Math.min(
        ...list.map(([i, j]) => deltaE(parseHex(palette[i]!.hex), parseHex(palette[j]!.hex), 'tritan'))
      )
...
export function ordinalRange(mode: Mode): SequentialSwatch[] {
  ...
  return buildSequential().filter((swatch) =>
    mode === 'light' ? swatch.step >= clamp : swatch.step <= clamp
  )
}
