...
import {ELEVATION_ROLE_COUNT, SHADOW_PRIMITIVE_COUNT, elevationCss, elevationTokens} from './elevation.js';
import {LAYOUT_PRIMITIVE_COUNTS, layoutCss, layoutTokens} from './layout.js';
import {MOTION_PRIMITIVE_COUNTS, motionCss, motionTokens} from './motion.js';
import {SPACING_ROLE_COUNT, spacingRoleCss, spacingRoleTokens} from './spacing-roles.js';
...
import {TYPOGRAPHY_PRIMITIVE_COUNT, typographyCss, typographyTokens} from './typography.js';
import {
...
function colorValue(l: number, c: number, h: number, hex: string, alpha: number = 1): ColorToken['$value'] {
  return {
    colorSpace: 'oklch',
    components: [Number(l.toFixed(PLACES)), Number(c.toFixed(PLACES)), Number(h.toFixed(PLACES))],
    alpha,
    hex,
  };
}
...
const SCALE_DESCRIPTIONS: Partial<Record<string, string>> = {
  ...
  danger:
    'The danger scale: solid fill, plus the tinted triple built from it — the destructive button and error states.',
  ...
  decorative:
    'The decorative scale: solid fill, plus the tinted triple built from it — colour with no semantic meaning.',
};
