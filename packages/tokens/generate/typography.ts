...
import {FONT_FAMILIES, FONT_SIZES, FONT_WEIGHTS, LETTER_SPACINGS, LINE_HEIGHTS} from '../config/typography.js';
...
export function typographyCss(): string {
  ...
  const sizes = Object.entries(FONT_SIZES).map(([name, value]) => `  --lat-font-size-${name}: ${value}rem;`);
  const lineHeights = Object.entries(LINE_HEIGHTS).map(([name, value]) => `  --lat-line-height-${name}: ${value};`);
  const letterSpacings = Object.entries(LETTER_SPACINGS).map(
    ([name, value]) => `  --lat-letter-spacing-${name}: ${value}em;`,
  );
  const weights = Object.entries(FONT_WEIGHTS).map(([name, value]) => `  --lat-font-weight-${name}: ${value};`);
...
export function typographyTokens(): TypographyTokenGroups {
  ...
  const lineHeight = Object.fromEntries(
    Object.entries(LINE_HEIGHTS).map(([name, value]) => [name, {$type: 'number', $value: value} satisfies NumberToken]),
  );
