...
| light accent as text on `bg` | 2.94:1 | 4.5:1 |
| light accent text on its own tint | 3.19:1 | 4.5:1 |
...
[`.github/workflows/release.yml`](./.github/workflows/release.yml) re-runs the whole contract suite on the tagged commit, packs both packages, verifies the tarballs, publishes them to npm with provenance, and opens a GitHub Release. It holds no npm credential. Publishing uses [trusted publishing](https://docs.npmjs.com/trusted-publishers): npm exchanges the OIDC token GitHub mints for the job against a publisher configured on each package — `chameleon-labs/lattice`, `release.yml`, environment `npm-publish` — and attests provenance automatically. There is no token to leak, rotate or expire.
